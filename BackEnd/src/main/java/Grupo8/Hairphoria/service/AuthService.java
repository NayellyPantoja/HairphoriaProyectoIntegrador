    ...
    public ValidateMessageResponse validateUser(String code, HttpServletResponse response) {
        try {
            ...
            if (LocalDateTime.now().isAfter(date)) {
                response.sendRedirect("http://s3-hairphoria-front.s3-website.us-east-1.amazonaws.com/registerconfirmdisable");
            ...
            if (usuario.isEnabled()) {
                response.sendRedirect("http://s3-hairphoria-front.s3-website.us-east-1.amazonaws.com/registerconfirm");
            ...
            throw new RuntimeException("Error al redireccionar a frontend");
        }
